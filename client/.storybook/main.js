...
const config = {
  ...
  viteFinal: async (config) => {
    ...
    // Add React plugin with classic JSX runtime at the start (required for styled-components v4)
    ...
    // Force formik to pre-bundle via CJS to prevent context mismatch with React 16
    config.optimizeDeps = config.optimizeDeps || {};
    config.optimizeDeps.include = [
      ...(config.optimizeDeps.include || []),
      'formik',
    ];
    // Deduplicate React instances to ensure single context tree
