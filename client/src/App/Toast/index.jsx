...
// Maps toast type to its corresponding icon name for visual feedback
// Icons help users quickly recognize the toast type at a glance
const typeIconMap = {
  success: 'feedback',
  danger: 'close',
  warning: 'issues',
        ...
        {toasts.map(toast => (
          <CSSTransition key={toast.id} classNames="jira-toast" timeout={200}>
            <StyledToast key={toast.id} type={toast.type} onClick={() => removeToast(toast.id)}>
              <TypeIcon type={typeIconMap[toast.type] || 'feedback'} size={18} />
              <Content>
